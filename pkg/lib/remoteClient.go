...
// copy from github.com/m3db/prometheus_remote_client_golang/promremote
import (
	"bytes"
	"context"
	"errors"
...
const (
	// DefaultRemoteWrite is the default Prom remote write endpoint in M3 Coordinator.
...
func (c *client) WriteProto(
	...
	return c.WriteRaw(ctx, data, opts)
}
...
func ToPromWriteRequest(t []TimeSeries) *prompb.WriteRequest {
	promTS := make([]prompb.TimeSeries, len(t))
